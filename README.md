...
## 4. `wrapperStream` 생성 (JSON 파싱 및 시간 포맷 변환)
```java
...
## 5. 3단계~6단계: `Sinks.mapWithEntryProcessor` (분기 처리형 대상 맵 반영)
...
### 5.1. 3단계/4단계: `M_SYSSE001I`, `M_SYSSE002I` (캐싱 목적 맵)
...
### 5.2. 5단계: `M_SYSSE014I` (이력 POJO 업데이트)
...
### 5.3. 6단계: `M_SYSSE015I` (시간대별 로그인 카운트 집계)
...
## 1. 동작 원리
### 1.1. **이벤트 감지**: 1단계 Source(`EventJournalMapEvent`)는 `ADDED` 이벤트 뿐만 아니라 `UPDATED` 이벤트도 모두 캡처하여 스트림으로 흘려보냅니다.
### 1.2. **상태 갱신 및 차단 로직**: 2단계 `mapStateful` 연산자는 `sessionId`를 키로 하여 기존에 쥐고 있던 메모리 객체(`SessionInfo`)를 꺼냅니다.
...
## 2. 3단계 (`M_SYSSE001I`): 고스트 업데이트로 인한 엔트리 부활 방지
...
## 3. 5단계 (`M_SYSSE014I`): 동일 세션ID 재로그인 허용
...
## 4. 6단계 (`M_SYSSE015I`): 사전 필터링 + fan-out 중복 제거
...
# (부록) Hazelcast Jet Job 자동 재시작 및 Bad Record Skip 방법

파이프라인 처리 중 예외가 발생했을 때 Job 전체가 `FAILED` 상태로 전환되지 않도록 하고, 실패한 레코드는 버리고 이후 이벤트는 계속 처리하는 전략을 설명합니다.

## 1. Jet Job 실패 유형과 자동 재시작 가능 여부

Hazelcast Jet의 Job 재시작 메커니즘은 실패 유형에 따라 동작이 다릅니다.

| 실패 유형 | ProcessingGuarantee로 해결? | 설명 |
|---------|--------------------------|------|
| **인프라 장애** (노드 다운, 네트워크 단절) | O | 스냅샷에서 복구 후 자동 재시작 |
| **User Code 예외** (NPE, ClassCastException 등) | **X** | FAILED 상태 고착, 동일 레코드 재처리 → 무한 루프 |

> **핵심**: "실패한 건 버리고 이후 처리"는 `ProcessingGuarantee`만으로는 불가능합니다. User Code 예외는 반드시 코드 레벨에서 방어해야 합니다.

---

## 2. 방법 1: try-catch + null 반환 (가장 근본적인 해결책)

실패한 이벤트를 "버리는" 유일한 확실한 방법은 **각 단계에서 예외를 잡아 null을 반환**하는 것입니다.

### 2.1. 현재 코드의 예외 처리 현황

| 단계 | 위치 | try-catch 여부 | 위험도 |
|------|------|---------------|-------|
| 1.5단계 | `mapUsingService` (역직렬화) | **없음** | 높음 |
| 2단계 | `mapStateful` | 있음 (null 반환) | 낮음 |
| 3단계 sink | EntryProcessor 람다 | **없음** | 높음 |
| wrapperStream | `mapUsingService` | 있음 (null 반환) | 낮음 |
| 4단계 sink | EntryProcessor 람다 | **없음** | 중간 |
| 5단계 sink | EntryProcessor 람다 | **없음** | 중간 |
| 6단계 sink | EntryProcessor 람다 | **없음** | 중간 |

### 2.2. 적용 패턴

**1.5단계 — `mapUsingService`**
```java
StreamStage<ExtractedSessionEvent> extractedStream = source.mapUsingService(ssFactory, (ss, event) -> {
    try {
        // ... 기존 로직
    } catch (Exception e) {
        log.error("[SessionJetPipeline] Step 1.5 skip - key={}", event.getKey(), e);
        return null; // 이 이벤트만 버림, Job은 계속
    }
}).filter(e -> e != null);
```

**3~6단계 — EntryProcessor 람다 (모두 동일 패턴)**
```java
return (com.hazelcast.map.EntryProcessor<String, SYSSE001I, Void>) entry -> {
    try {
        // ... 기존 로직
    } catch (Exception e) {
        log.error("[SESSION_AUDIT] EntryProcessor skip - key={}", sessionId, e);
        return null; // 해당 엔트리만 skip, Job은 계속
    }
};
```
...
## 3. 방법 2: ProcessingGuarantee + Snapshot (인프라 장애 대응)

User Code 예외에는 효과가 없지만, **노드 장애 시 재시작**을 보장하기 위한 보완 설정입니다.
...
// 노드 장애 시 마지막 스냅샷에서 재시작
jobConfig.setProcessingGuarantee(
    com.hazelcast.jet.config.ProcessingGuarantee.AT_LEAST_ONCE
);
jobConfig.setSnapshotIntervalMillis(30_000); // 30초마다 스냅샷 저장
```

> **주의**: `ProcessingGuarantee`만 설정하고 방법 1(try-catch)을 적용하지 않으면, User Code 예외로 FAILED된 Job이 재시작되어도 동일한 bad record를 다시 처리하다 또 실패하는 **무한 루프**에 빠집니다.

---

## 4. 방법 3: Watchdog 패턴 (FAILED 감지 후 자동 재제출)

방법 1, 2를 모두 적용한 후에도 예기치 못한 이유로 Job이 FAILED 상태가 된 경우, **외부에서 감시하다 자동으로 재제출**합니다.

```java
@Component
@RequiredArgsConstructor
@Slf4j
public class SessionJetJobWatchdog {

    private final HazelcastInstance hazelcastInstance;
    private final SessionJetPipelineConfig pipelineConfig;

    @Scheduled(fixedDelay = 30_000) // 30초마다 상태 점검
    public void watchAndRestart() {
        try {
            JetService jet = hazelcastInstance.getJet();
            com.hazelcast.jet.Job job = jet.getJob("SessionJetPipelineJob");

            if (job == null) {
                log.warn("[SessionJetWatchdog] Job not found. Resubmitting...");
                pipelineConfig.initPipeline();
                return;
            }

            com.hazelcast.jet.core.JobStatus status = job.getStatus();
            log.debug("[SessionJetWatchdog] Job status: {}", status);

            if (status == com.hazelcast.jet.core.JobStatus.FAILED) {
                log.warn("[SessionJetWatchdog] Job FAILED detected. Resubmitting...");
                try { job.cancel(); } catch (Exception ignored) {}
                pipelineConfig.initPipeline();
            }

        } catch (Exception e) {
            log.error("[SessionJetWatchdog] Watchdog check failed", e);
        }
    }
}
```
...
## 5. 세 가지 방법의 조합 전략 및 요약

```
[방법 1] try-catch + null 반환
        ↓
    Bad Record → 로그 기록 후 skip → Job 계속 실행  ← 핵심 (필수)

[방법 2] ProcessingGuarantee.AT_LEAST_ONCE
        ↓
    노드 다운 → 마지막 스냅샷에서 복구 후 재시작

[방법 3] Watchdog @Scheduled
        ↓
    방법 1, 2를 뚫고 FAILED된 경우 → 자동 재제출
```

| 목표 | 해결책 |
|------|-------|
| **"실패한 건 버리고 다음 이벤트 계속 처리"** | **방법 1 필수** (try-catch + null) |
| 노드 장애 시 자동 복구 | 방법 2 (ProcessingGuarantee) |
| 예기치 못한 FAILED 상태 자동 복구 | 방법 3 (Watchdog) |

> **방법 1 없이 방법 3만 사용하면**, FAILED 원인인 동일한 bad record를 계속 재처리하여 무한 재시작 루프에 빠집니다. **방법 1이 선행 조건**입니다.
